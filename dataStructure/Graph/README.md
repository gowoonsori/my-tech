...
| 데이크스트라 알고리즘 (Dijkstra's algorithm) | O( \|E\| log\|V\| ) | O( \|V\|^2 ) | O( \|V\| + \|E\| ) |
| A* 알고리즘 (A* search algorithm) | O( \|E\| ) | O( b^d ) |O( b^d ) |
| 프림 알고리즘 (Prim's algorithm)) |  O( \|E\| log\|V\| ) | O( \|V\|^2 ) | O( \|V\| + \|E\| ) |
| 벨먼-포드 알고리즘 (Bellman-Ford algorithm) | O( \|E\| * \|V\| ) |  O( \|E\| * \|V\| ) | O( \|V\| )
| 플로이드-워셜 알고리즘 (Floyd-Warshall algorithm) | O( \|V\|^3 ) | O( \|V\|^3 ) | O( \|V\|^2 ) |
| 위상 정렬 (Topological sort) | O( \|V\| + \|E\| ) | O( \|V\| + \|E\| ) | O( \|V\| + \|E\| ) |
