...
- 서버 : 서버의 부담이 증가하고, JWT는 휘발성 데이터이다.

> 웹 스토리지는 로컬 스토리지와 세션스토리지로 나누어지고 로컬 스토리지는 창이 꺼져도 데이터가 유지 되고 세션스토리지는 창이 꺼지면 데이터가 지워진다.
>
...
1. 쿠키에 저장하는 방식도 탈취가 가능하지만 옵션설정으로 공격을 어느정도 무력화 시킬 수 있다. (`XSS`공격에 안전한 대신 `CSRF`에 비교적 취약하다)
